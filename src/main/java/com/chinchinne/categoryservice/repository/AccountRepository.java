...
{
    @Query("SELECT CASE WHEN COUNT(a) > 0 THEN TRUE ELSE FALSE END FROM Account a WHERE a.userId = :userId AND a.categoryId IN (:categoryIds) AND a.delYn = :delYn")
    boolean existByCategoryIdIn(@Param("userId") UserId userId, @Param("categoryIds") List<CategoryId> categoryIds, @Param("delYn") Common delYn);
}
